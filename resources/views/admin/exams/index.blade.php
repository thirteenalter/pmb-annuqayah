        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-0">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                @foreach ($exams as $exam)
                    <div
                        class="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow flex flex-col justify-between">
                        <div>
                        ...
                        <div class="space-y-2">
                            ...
                            <form action="{{ route('admin.exams.destroy', $exam->id) }}" method="POST"
                                onsubmit="return confirm('Hapus ujian ini? Data hasil tes siswa juga mungkin akan berpengaruh.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="w-full text-center py-2 text-[10px] font-black uppercase tracking-widest text-red-400 hover:text-red-600 transition-colors">
                                    Hapus Ujian
                                </button>
                            </form>
                        </div>
