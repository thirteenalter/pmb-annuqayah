             ...
             <div class="max-w-7xl mx-auto lg:px-0 px-4">
                 <div class="flex items-center gap-3 py-4 overflow-x-auto no-scrollbar outline-none">
                     ...
                     <a href="{{ route('formulir') }}"
                         ...
                         <span class="text-sm font-bold whitespace-nowrap">Daftar</span>
                     </a>

                     @if ($user->registration_period_id != null)
                         <a href="{{ route('student.index') }}"
                             class="flex-none flex items-center gap-2 px-5 py-2.5 rounded-xl border transition-all duration-200 {{ request()->routeIs('student.index') ? 'bg-slate-500 border-slate-700 text-white shadow-lg shadow-slate-100' : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-white hover:border-indigo-400 hover:text-indigo-600' }}">
                     ...
                     @if ($user->isDataLengkap())
                         ...
                         <a href="{{ route('exams.index') }}"
                             class="flex-none flex items-center gap-2 px-5 py-2.5 rounded-xl border transition-all duration-200 {{ request()->routeIs('exams.index') ? 'bg-slate-500 border-slate-700 text-white shadow-lg shadow-slate-100' : 'bg-slate-50 border-slate-200 text-slate-600 hover:bg-white hover:border-indigo-400 hover:text-indigo-600' }}">
                             <span class="material-symbols-outlined !text-[20px]">assignment</span>
                             <span class="text-sm font-bold whitespace-nowrap">UJIAN / CBT</span>
                         </a>
                     @endif
